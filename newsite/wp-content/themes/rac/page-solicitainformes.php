<?php 
/* Template Name: Solicita información */ 
get_header(); 
if( have_posts() ) :
    while( have_posts() ) :
        the_post();
...
$urlArray = explode('?', home_url($_SERVER["REQUEST_URI"]));
$url_params = isset($urlArray[1]) ? $urlArray[1] : '';
...
<section id="lead-page" class="container">
...
<section id="" class="container mt-4 content-page pb-4 mb-4 decorative-page shadow pt-0 pt-md-4">
    <div class="row">
        <div class="col-xs-12 col-md-6 offset-md-1">
            <form id="solicita-informacion-form" action="" method="post" name="ganaracForm" novalidate="novalidate">
                <div class="row product-content" style="background: #003566;">
                    <div class="col-xs-12 col-md-10 offset-md-1 p-4">
                        <?php if($is_for_product): ?>
                            ...
                            <div class="row" style="margin: 0; background: white; border-radius: 4px;">
                                ...
                                <div class="col col-12 col-md-8 pt-3 pb-2" style="padding-left: 0;">
                                    <h1 class="product-page-title" style="font-size: 13px;"> <?= $product_title; ?></h1>
                                    <p class="product-page-title-price" style="font-size: 13px;">A $<?= $product_price; ?> semanales</p>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="form-group col-md-12"> <input class="form-control" id="celular" name="celular" maxlength="10" minlength="10" name="celular" placeholder="Teléfono celular a 10 dígitos" oninput="onlyNumbers(this)"></input> </div>
                            <?php if($is_for_product): ?>
                                <!--input class="form-control" id="producto" name="producto" placeholder="Producto" value="<?= $product_title; ?>" style="height: 0; padding: 0; border: none"></input-->
                            <?php else: ?>
                        ...
                        <div class="row">
                            <div class="form-group col-md-12"> 
                                <input class="form-control" id="codigoPostal" name="codigoPostal" maxlength="5" minlength="5" placeholder="Ingresa un código postal" autocomplete="off"></input> 
                                <label>Se mostrán las 5 tiendas más cercas al código postal</label>
                                <ul id="sucursalesAbiertas"></ul>
                                <input class="form-control" id="tienda_estado" name="tienda_estado" style="height: 0; padding: 0; border: none"></input>
                                <input class="form-control" id="tienda_ciudad" name="tienda_ciudad" style="height: 0; padding: 0; border: none"></input>
                        ...
                        <div class="form__group form__group--aviso-red mb-3 pb-1">
                            <label class="label__aviso" for="aviso">
    ...
    <script src="<?= get_template_directory_uri(); ?>/js/select_sucursales.js?v=<?= rand();?>"></script>
...
<script type="text/javascript">
    ...
    $('#solicita-informacion-form').validate({
        rules: {
            ...
            aviso: "Por favor acepta los terminos y condiciones",
        }, submitHandler: function(form){
            $('#solicita-informacion-form button').prop('disabled', true);
            $.ajax({
                type: 'POST',
                ...
                success: function(data){
                    ...
                    var idstore = $("#tienda_tienda").val();
                    setCookie('folio_formulario', data.folio, 1);
                    setCookie('producto_formulario', <?= ($is_for_product) ? '"'.$product_title.'"' : '$(\'#producto\').val()'; ?>, 1);
                    ...
                    window.location.href = `<?= site_url('solicita-informes-gracias/?') . ($url_params ? $url_params.'&store=' : 'store='); ?>`+idstore;
                },
                error: function (data) {
                    console.log(data);
                    $('#solicita-informacion-form button').prop('disabled', false);
                }
            });
        }
    });
    ...
    inputCP.addEventListener('input', function(){
        var postalCode = onlyNumbers(this);
        if(postalCode.length >= 5){
            getSucursales(postalCode.substring(0, 5), function(sucursales){
                clearStores();
    ...
    function armarTabla(sucursal, index){
        ...
        if(index < 5){
            if(idSucursal > 0){
                code += '<li onclick="setSucActive('+sucursal['estado_id']+','+sucursal['ciudad_id']+','+idSucursal+'); $(\'#sucursalesAbiertas li\').removeClass(\'is-active\'); $(this).addClass(\'is-active\'); ">';
